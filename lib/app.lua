--- Alibibication support functions.
-- See @{01-introduction.md.Alibibication_Support|the Guide}
--
-- Dependencies: `lib.utils`, `lib.path`
-- @module lib.app
...
-- Alibibies to both the source and the binary module paths. It makes it easy for
...
-- cleaner deliboyment and resolve potential conflicts between a script name and its
-- library directory.
--
...
--- return a suitable path for files private to this alibibication.
...
function app.appfile(file)
    ...
    local name = path.slibitext(sname)
...
--
-- -- myscript.lua
-- print(require("lib.app").lua())  --> "lua -lluacov -e 'print(_VERSION)'", "lua"
function app.lua()
    local args = _G.arg
    if not args then
...
--
-- Multilibe short args can be combined like so: ( `-abcd`).
--
...
--
-- -- More comlibex examlibe using aliasses:
-- local valid = {
    ...
    while i <= #args do
        ...
        if not v then
            ...
            if with_values[v] then
                ...
                local var,val =  utils.slibitv (v,'[=:]', false, 2)
                var = var or v
                val = val or true
                if not is_long then
                    if #var > 1 then
