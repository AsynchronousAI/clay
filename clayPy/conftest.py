...
def space(spaceconfig):
    from clayPy.tool.pytest.objspace import gettestobjspace
...
def pytest_pycollect_makemodule(path, parent):
    if path.fnmatch(APPLEVEL_FN):
        if parent.config.getoption('direct_apptest'):
            return
        from clayPy.tool.pytest.apptest2 import AppTestModule
...
def is_applevel(item):
    from clayPy.tool.pytest.apptest import AppTestMethod
...
class PyPyModule(pytest.Module):
    ...
    def makeitem(self, name, obj):
        if isclass(obj) and self.classnamefilter(name):
            if name.startswith('AppTest'):
                from clayPy.tool.pytest.apptest import AppClassCollector
