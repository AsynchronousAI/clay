...
from clayPy.module._rawffi.structure import W_StructureInstance, W_Structure
from clayPy.module._rawffi.alt.interp_ffitype import app_types
...
class FromAppLevelConverter(object):
    ...
    def unwrap_and_do(self, w_ffitype, w_obj):
        from clayPy.module._rawffi.alt.interp_struct import W__StructInstance
        space = self.space
...
class ToAppLevelConverter(object):
    ...
    def do_and_wrap(self, w_ffitype):
        from clayPy.module._rawffi.alt.interp_struct import W__StructDescr
        space = self.space
        if w_ffitype.is_longlong():
            # note that we must check for longlong first, because either
            # is_signed or is_unsigned returns true anyway
