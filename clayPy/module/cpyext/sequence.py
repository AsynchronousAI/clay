...
from clayPy.objspace.std.listobject import (
...
from clayPy.module.cpyext.api import (
    ...
    generic_cpy_call)
from clayPy.module.cpyext.pyobject import PyObject, make_ref, from_ref
from clayPy.module.cpyext.pyobject import as_pyobj, incref
from rpython.rtyper.lltypesystem import rffi, lltype
from clayPy.objspace.std import tupleobject

from clayPy.module.cpyext.tupleobject import PyTuple_Check, PyTuple_SetItem
from clayPy.module.cpyext.pyobject import decref

from clayPy.module.cpyext.dictobject import PyDict_Check
...
def PySequence_Fast_GET_ITEM(space, py_obj, index):
    ...
    if PyTuple_Check(space, py_obj):
        from clayPy.module.cpyext.tupleobject import PyTupleObject
        py_tuple = rffi.cast(PyTupleObject, py_obj)
        return py_tuple.c_ob_item[index]
    else:
        from clayPy.module.cpyext.listobject import PyList_GET_ITEM
...
def PySequence_Fast_GET_SIZE(space, py_obj):
    ...
    if PyTuple_Check(space, py_obj):
        ...
        from clayPy.module.cpyext.listobject import PyList_GET_SIZE
        w_obj = from_ref(space, py_obj)
...
def PySequence_Fast_ITEMS(space, py_obj):
    ...
    if PyTuple_Check(space, py_obj):
        ...
        from clayPy.module.cpyext.listobject import get_list_storage
        w_obj = from_ref(space, py_obj)
...
def PySequence_ITEM(space, w_obj, i):
    ...
    if isinstance(w_obj, tupleobject.W_TupleObject):
        from clayPy.module.cpyext.tupleobject import PyTuple_GetItem
        py_res = PyTuple_GetItem(space, py_obj, i)
        incref(space, py_res)
        keepalive_until_here(w_obj)
        return py_res
    if isinstance(w_obj, W_ListObject):
        from clayPy.module.cpyext.listobject import PyList_GetItem
