...
from clayPy.interpreter.nestedscope import Cell
from clayPy.interpreter.module import Module
from clayPy.interpreter.function import StaticMethod
from clayPy.objspace.std.sliceobject import W_SliceObject
from clayPy.module.__builtin__.descriptor import W_Property
from clayPy.module.__builtin__.interp_classobj import W_ClassObject
from clayPy.module.micronumpy.base import W_NDimArray
from clayPy.module.__pypy__.interp_buffer import W_Bufferable
from rpython.rlib.entrypoint import entrypoint_lowlevel
from rpython.rlib.rposix import SuppressIPH
from rpython.rlib.unroll import unrolling_iterable
from rpython.rlib.objectmodel import specialize
from clayPy.module.exceptions import interp_exceptions
...
from clayPy.objspace.std.typeobject import W_TypeObject, find_best_base
...
class ApiFunction(BaseApiFunction):
    ...
    def get_unwrapper(self):
        ...
        def unwrapper(space, *args):
            from clayPy.module.cpyext.pyobject import is_pyobj
            from clayPy.module.cpyext.pyobject import from_ref, as_pyobj
            newargs = ()
            keepalives = ()
...
def build_exported_objects():
    ...
    from clayPy.module.exceptions.moduledef import Module as ExcModule
...
def build_type_checkers_flags(type_name, cls=None, flagsubstr=None):
    ...
    def check(space, pto):
        from clayPy.module.cpyext.pyobject import is_pyobj, as_pyobj
        "Implements the Py_Xxx_Check function"
        if is_pyobj(pto):
...
def _restore_gil_state(pygilstate_release, gilstate, gil_release, _gil_auto):
    ...
    if pygilstate_release:
        from clayPy.module.cpyext import pystate
...
def make_wrapper_second_level(space, argtypesw, restype,
    ...
    def wrapper_second_level(callable, pname, *args):
        from clayPy.module.cpyext.pyobject import make_ref, from_ref, is_pyobj
        from clayPy.module.cpyext.pyobject import as_pyobj
        from clayPy.module.cpyext import pystate
...
def setup_init_functions(eci, prefix):
    ...
    def init_types(space):
        from clayPy.module.cpyext.typeobject import py_type_ready
        from clayPy.module.sys.interp_encoding import getfilesystemencoding
    ...
    from clayPy.module.posix.interp_posix import add_fork_hook
...
def build_bridge(space):
    ...
    for name, (typ, expr) in GLOBALS.iteritems():
        ...
        from clayPy.module import cpyext    # for the eval() below
        w_obj = eval(expr)
...
def attach_recursively(space, static_pyobjs, static_objs_w, attached_objs, i):
    ...
    from clayPy.module.cpyext.pyobject import get_typedescr, make_ref
...
class StaticObjectBuilder(object):
    ...
    def attach_all(self, space):
        ...
        from clayPy.module.cpyext.typeobject import finish_type_1, finish_type_2
        from clayPy.module.cpyext.pyobject import track_reference
        #
...
def setup_micronumpy(space):
    ...
    from clayPy.module.cpyext.ndarrayobject import HEADER
...
def load_extension_module(space, path, name):
    ...
    if space.config.objspace.usemodules._cffi_backend:
        ...
        try:
            ...
            try:
                from clayPy.module._cffi_backend import cffi1_module
...
def make_generic_cpy_call(FT, expect_null):
    from clayPy.module.cpyext.pyobject import is_pyobj, make_ref, decref
    from clayPy.module.cpyext.pyobject import get_w_obj_and_decref
    from clayPy.module.cpyext.pyerrors import PyErr_Occurred
