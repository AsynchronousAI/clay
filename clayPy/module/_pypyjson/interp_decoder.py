...
from clayPy.module._pypyjson import simd
...
class JSONDecoder(W_Root):
    ...
    def _create_dict_map(self, values_w, jsonmap):
        from clayPy.objspace.std.jsondict import from_values_and_jsonmap
        return from_values_and_jsonmap(self.space, values_w, jsonmap)

    def _devolve_jsonmap_dict(self, w_dict):
        from clayPy.objspace.std.jsondict import devolve_jsonmap_dict
        devolve_jsonmap_dict(w_dict)

    def _get_jsonmap_from_dict(self, w_dict):
        from clayPy.objspace.std.jsondict import get_jsonmap_from_dict
    ...
    def _create_dict(self, d):
        from clayPy.objspace.std.dictmultiobject import from_unicode_key_dict
        return from_unicode_key_dict(self.space, d)

    def _create_empty_dict(self):
        from clayPy.objspace.std.dictmultiobject import create_empty_unicode_key_dict
...
class MapBase(object):
    ...
    def get_next(self, w_key, string, start, stop, terminator):
        ...
        """
        from clayPy.objspace.std.dictmultiobject import unicode_hash, unicode_eq
