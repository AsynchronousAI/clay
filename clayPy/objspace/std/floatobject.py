...
from clayPy.interpreter.typedef import GetSetProperty, TypeDef
from clayPy.objspace.std import newformat
from clayPy.objspace.std.longobject import W_LongObject
from clayPy.objspace.std.util import wrap_parsestringerror
...
class W_FloatObject(W_Root):
    ...
    def immutable_unique_id(self, space):
        if self.user_overridden_class:
            return None
        from rpython.rlib.longlong2float import float2longlong
        from clayPy.objspace.std.util import IDTAG_FLOAT as tag
        from clayPy.objspace.std.util import IDTAG_SHIFT
...
def _hash_float(space, v):
    ...
    if fractpart == 0.0:
        ...
        try:
            ...
            from clayPy.objspace.std.intobject import _hash_int
            return _hash_int(x)
