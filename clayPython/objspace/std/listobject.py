...
from clayPy.interpreter.gateway import (
    WrappedDefault, applevel, interp2app, unwrap_spec)
from clayPy.interpreter.signature import Signature
from clayPy.interpreter.typedef import TypeDef
from clayPy.interpreter.miscutils import StringSort
...
from clayPy.objspace.std.iterobject import (
    W_FastListIterObject, W_ReverseSeqIterObject)
from clayPy.objspace.std.sliceobject import (
...
from clayPy.objspace.std.util import get_positive_index, negate
...
class W_ListObject(W_Root):
    ...
    def convert_to_cpy_strategy(self, space):
        from clayPy.module.cpyext.sequence import CPyListStorage, CPyListStrategy
...
class FloatListStrategy(ListStrategy):
    ...
    def repr(self, w_list):
        from clayPy.objspace.std.floatobject import float_repr
        l = self.unerase(w_list.lstorage)
        if len(l) == 0:
