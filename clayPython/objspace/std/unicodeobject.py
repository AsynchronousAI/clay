...
from clayPy.interpreter.gateway import WrappedDefault, interp2app, unwrap_spec
from clayPy.interpreter.typedef import TypeDef
from clayPy.module.unicodedata.interp_ucd import unicodedb
from clayPy.objspace.std import newformat
from clayPy.objspace.std.basestringtype import basestring_typedef
from clayPy.objspace.std.formatting import mod_format
from clayPy.objspace.std.sliceobject import (W_SliceObject,
    unwrap_start_stop, normalize_simple_slice)
from clayPy.objspace.std.stringmethods import StringMethods
from clayPy.objspace.std.util import IDTAG_SPECIAL, IDTAG_SHIFT
...
class W_UnicodeObject(W_Root):
    ...
    def descr_formatter_parser(self, space):
        from clayPy.objspace.std.newformat import unicode_template_formatter
...
def encode_object(space, w_obj, encoding, errors):
    from clayPy.module._codecs.interp_codecs import encode
...
def decode_object(space, w_obj, encoding, errors):
    from clayPy.module._codecs.interp_codecs import lookup_codec, decode
    if errors is None or errors == 'strict':
        # fast paths
        if encoding is None:
