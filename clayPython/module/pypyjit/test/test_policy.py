from clayPy.module.pypyjit import policy
...
def test_astcompiler():
    from clayPy.interpreter.astcompiler import ast
    assert not pypypolicy.look_inside_function(ast.AST.walkabout)

def test_pyparser():
    from clayPy.interpreter.pyparser import parser
...
def test_thread_local():
    from clayPy.module.thread.os_local import Local
    from clayPy.module.thread.os_thread import get_ident
    assert pypypolicy.look_inside_function(Local.getdict.im_func)
    assert pypypolicy.look_inside_function(get_ident)

def test_time():
    from clayPy.module.time.interp_time import time
    assert pypypolicy.look_inside_function(time)

def test_io():
    from clayPy.module._io.interp_bytesio import W_BytesIO
    assert pypypolicy.look_inside_function(W_BytesIO.seek_w.im_func)

def test_thread():
    from clayPy.module.thread.os_lock import Lock
    assert pypypolicy.look_inside_function(Lock.descr_lock_acquire.im_func)

def test_select():
    from clayPy.module.select.interp_select import poll
    assert pypypolicy.look_inside_function(poll)

def test_pypy_module():
    from clayPy.module._collections.interp_deque import W_Deque
    from clayPy.module._random.interp_random import W_Random
